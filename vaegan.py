...
class vaegan(object):
    ...
    def build_model_vaegan(self):
        ...
        self.D_fake_loss = tf.reduce_mean(
            tf.nn.sigmoid_cross_entropy_with_logits(labels=tf.zeros_like(self.G_pro_logits), logits=self.G_pro_logits), name="D_fake_loss")
        ...
        self.D_tilde_loss = tf.reduce_mean(
            tf.nn.sigmoid_cross_entropy_with_logits(labels=tf.zeros_like(self.De_pro_tilde), logits=self.De_pro_tilde), name="D_tilde_loss")

        tf.add_to_collection("D_fake_loss", self.D_fake_loss)
        tf.add_to_collection("D_real_loss", self.D_real_loss)
        tf.add_to_collection("D_tilde_loss", self.D_tilde_loss)
        ...
        tf.add_to_collection("G_fake_loss", self.G_fake_loss)
        tf.add_to_collection("G_tilde_loss", self.G_tilde_loss)
